...
class PSBL(PSPL):
    ...
    def lens_map(self, t_obs, z):
        ...
        z1, z2 = self.get_resolved_lens_astrometry(t_obs) # Normalized lens positions
        z1 = (z1[...,0] + z1[...,1]*1j)/R # Complex position of lens 1
        z2 = (z2[...,0] + z2[...,1]*1j)/R # Complex position of lens 2
        w_arr = np.zeros(np.shape(t_obs)+np.shape(z[0]),dtype = np.complex128)
        for i, _ in np.ndenumerate(t_obs):
            w_arr[i] = z[i] - m1/np.conj(z[i] - z1[i]) - m2/np.conj(z[i] - z2[i])
        return w_arr

    def get_critical_curves(self, t_obs, N_pts=2000, amp_inv=0.0):
        '''
        ...
        z_arr = np.zeros(np.shape(t_obs)+(N_pts,4),dtype = np.complex128) # Stores points along curve
        for i, _ in np.ndenumerate(t_obs):
    ...
    def get_caustics(self, t_obs, N_pts=2000, amp_inv=0):
        '''
        ...
        w_arr = self.lens_map(t_obs,self.get_critical_curves(t_obs, N_pts, amp_inv))
        return w_arr
