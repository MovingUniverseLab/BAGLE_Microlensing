...
def test_PSPL_other(plot=False):
    ...
    run_test_PSPL(mL, t0, xS0, beta, muS, muL, dL, dS, b_sff, mag_src,
                  outdir=f'{TEST_OUTPUT_DIR}/test_pspl_other_jax/', plot=plot)
...
def test_PSPL_belokurov(plot=False):
    ...
    run_test_PSPL(mL, t0, xS0, beta, muS, muL, dL, dS, b_sff, mag_src,
                  outdir=f'{TEST_OUTPUT_DIR}/test_pspl_belokurov_jax/', plot=plot)
...
def compare_pspl_parallax_belokurov():
    outdir = f'{TEST_OUTPUT_DIR}/test_pspl_parallax_belokurov_jax/'
...
def compare_pspl_parallax_han2000():
    ...
    run_test_pspl_parallax(raL, decL, mL, t0, xS0, beta, muS, muL, dL, dS,
                           b_sff, imag, outdir=f'{TEST_OUTPUT_DIR}/test_pspl_parallax_han2000_jax/')
...
def compare_pspl_parallax_bulge1():
    ...
    run_test_pspl_parallax(raL, decL, mL, t0, xS0, beta, muS, muL, dL, dS,
                           b_sff, imag, outdir=f'{TEST_OUTPUT_DIR}/test_pspl_par_bulge1_jax/')
...
def compare_pspl_parallax_paczynski1998(t0=57000):
    ...
    outdir = f'{TEST_OUTPUT_DIR}/test_pspl_parallax_paczynski1998_jax/'
    if (outdir != '') and (outdir != None):
        os.makedirs(outdir, exist_ok=True)
...
def test_pspl_parallax2_bulge():
    outdir = f'{TEST_OUTPUT_DIR}/test_pspl_par2_bulge1_jax/'
...
def test_FSPL_PhotAstrom_source_astrometry(plot=False):
    ...
    tests_dir = os.path.dirname(os.path.realpath(__file__))
    outdir = os.path.join(TEST_OUTPUT_DIR, 'test_FSPL_source_astrometry_jax') + '/'
...
def test_spitzer_zang2020(plot=False):
    ...
    outdir = f'{TEST_OUTPUT_DIR}/test_pspl_par_spitzer_zang2020_jax/'
...
def test_spitzer_shvartzvald2019(plot=False):
    ...
    outdir = f'{TEST_OUTPUT_DIR}/test_pspl_par_spitzer_shvartzvald2019_jax/'
...
def test_pspl_luminous_lens(plot=False):
    outdir = f'{TEST_OUTPUT_DIR}/test_pspl_lumlens_jax/'
...
def test_pspl_phot_model_jax_get_astrometry_matches_numpy(class_name):
    """model_jax.get_astrometry vs bagle.model reference (Einstein radii)."""
    from model_old_vs_jax_fixtures import build_paired_instances, time_grid_ast

    ref_inst, jax_inst = build_paired_instances(class_name)
